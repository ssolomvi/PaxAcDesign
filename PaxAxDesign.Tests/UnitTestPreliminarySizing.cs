...
public class Tests
{
    ...
    {
        // new FileInfo($"{Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\"))}Resources\\pax-ac-design.db")
        string name = "Slat";
        var ds = $"{Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\..\"))}Resources\\pax-ac-design.db";
        ConnectionProvider _connectionProvider = ConnectionProvider.create(ds);
        ...
        using (var ts = connection.BeginTransaction())
        {
            ...
            using (var reader = selectCommand.ExecuteReader())
            {
                while (reader.Read()) // построчно считываем данные
                {
                    ...
                    Console.WriteLine(
                        $"{highLiftDevicesTable}\t{maxLiftCoefficientTable}\t{maxLiftCoefficientDeltaTable}");
                }
            }
        }
        // var command =
        //     _dbContext.CreateCommand($"SELECT * FROM `CL_max (Dubs1987)`");
    ...
    public void DatabaseFiltering()
    {
        ...
        Assert.Equals(maxLiftCoefficient, 2.0);
    }
